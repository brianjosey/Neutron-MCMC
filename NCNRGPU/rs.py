...
class CDataInteractor():
    ...
    def fnSaveSingleColumnsFromStatDict(self, sFilename, data, skipentries=[]):
        ...
        for i in range(len(list(data.values())[0]['Values'])):
            for element in data:
                if element not in skipentries:
...
class CReflectometry:
    ...
    def fnCreateBilayerPlotData(self):
        ...
        def fnStat(diarea, name, diStat):
            for i in range(len(list(diarea.values())[0])):
        ...
        for i in range(len(diIterations['substrate'])-1):
...
#-------------------------------------------------------------------------------
    ...
    def fnLoadStatData(self, sparse=0):
        ...
        self.diStatResults['NumberOfStatValues'] = \
            len(list(self.diStatResults['Parameters'].values())[0]['Values'])
    ...
    def fnPlotMolgroups(self, sPath):
        ...
        firstGroup = list(self.diMolgroups.values())[0]
        for i, zi in enumerate(firstGroup['zaxis']):
            File.write(str(zi) + ' ')
            summe = 0
            normarea = 0
            for element in self.diMolgroups:
    ...
    def fnPullMolgroupLoader(self, liMolgroupNames):
        ...
        zaxis = list(self.diStatResults['Molgroups'][0].values())[0]['zaxis']
        diarea = {'zaxis': zaxis}
        dinsl = {'zaxis': zaxis}
        dinsld = {'zaxis': zaxis}
...
def fMCMC(iMaxIterations=1024000, liMolgroups=['protein'], fSparse=0):
    while True:
        ...
        lCommand = ['/storage/conda/refl1d/bin/refl1d', 'run.py', '--fit=dream', '--parallel=0']
        if bMCMCexists:
